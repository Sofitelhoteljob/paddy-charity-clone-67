...
const HelpSection = () => {
  ...
  return <section className="py-20 bg-charity-green">
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {helpOptions.map((option, index) => <div key={index} className="bg-charity-dark p-8 rounded-lg text-center text-white">
              <div className="flex justify-center mb-4">
                {option.icon}
              </div>
              <h3 className="text-xl font-bold mb-3">{option.title}</h3>
              <p className="opacity-80 mb-6">{option.description}</p>
              <Button variant="outline" className="border-white hover:bg-white font-medium text-zinc-950">
                {option.buttonText}
              </Button>
            </div>)}
        </div>
