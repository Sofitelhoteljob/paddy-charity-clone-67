...
const FeatureCards = () => {
  ...
  return <div className="container mx-auto px-4 -mt-28 relative z-30">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {features.map((feature, index) => <div key={index} className={`${feature.background} p-8 rounded-lg text-white shadow-lg`}>
            <div className="flex items-center mb-6">
              {feature.icon}
              <h3 className="text-xl font-bold ml-4">{feature.title}</h3>
            </div>
            <p className="mb-6">{feature.description}</p>
            <Button variant="outline" className="border-white hover:bg-white transition-colors font-medium text-gray-950">
              {feature.buttonText}
            </Button>
          </div>)}
      </div>
